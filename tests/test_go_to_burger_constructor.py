...
class TestStellarBurgersGoToConstructor:
    ...
    def test_go_to_constructor_by_constructor_button_from_private_office(
            self, login):
        ...
        assert BurgerConstructorData.h1_in_burger_constructor in \
               burger_constructor,\
            f'Не обнаружен заголовок ' \
            f'"{BurgerConstructorData.h1_in_burger_constructor}"' \
            f' в Конструкторе бургеров'

    def test_go_to_constructor_by_stellar_burgers_logo_from_private_office(
            self, login):
        login.find_element(*Locators.PRIVATE_OFFICE_BUTTON).click()
        WebDriverWait(login, 5).until(
            EC.visibility_of_element_located(Locators.USER_PROFILE)).text
